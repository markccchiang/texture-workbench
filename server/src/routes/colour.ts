...
export const colourRoutes: FastifyPluginAsyncTypebox<ColourRoutesOptions> = async (app, { config, store }) => {
  ...
  async function decode(source: ImageInfo, conversion: ColourConversion, options: { firstSlice?: boolean; encodeTiff?: boolean }) {
    let decoded: native.DecodedImage & { tiff?: Buffer };
    try {
      decoded = await native.decodeImageFile(store.originalPath(source.imageId), {
      ...
      throw new ApiError(422, code === 'IMAGE_TOO_LARGE' ? 'ImageTooLarge' : 'InvalidImage', (error as Error).message);
    }
    // A stack made on the server (a DICOM series) keeps only its gray slices
  ...
  app.post(
    '/images/:id/colour',
    {
      schema: {
        ...
        description:
          'Decodes the colour image again with another conversion and stores the result as a new image named "<name> [<conversion>]", whose original file is an uncompressed TIFF of the converted slices (its SHA-256 identifies the image) and whose colourSource names the colour image. The conversion is recorded as the image\'s valueConversion, so results say what was measured. Called on an image that was converted already, it converts its colour image. luminance returns the colour image itself; converting an image the same way twice returns the image made the first time. 422 NotColour for gray images.',
        tags: ['images'],
    ...
    async (request, reply) => {
      ...
      if (existing) {
        return reply.code(200).send(withTransfer(existing));
      }

      const decoded = await decode(source, conversion, { encodeTiff: true });
  ...
  app.get(
    '/images/:id/colour-preview.png',
    {
      schema: {
        ...
        description:
          'The first slice of the colour image (or of the image it was converted from) converted with conversion, rendered with its own default window (0.5th–99.5th percentiles) and reduced so the long side is at most maxSize.',
        tags: ['images'],
        params: ImageIdParams,
    ...
    async (request, reply) => {
      const source = await colourImage(request.params.id);
      const decoded = await decode(source, request.query.conversion, { firstSlice: true });
      const maxSize = Math.min(request.query.maxSize ?? config.displayMaxSize, config.displayMaxSize);
      const png = await native.renderDisplay(decoded.pixels, decoded.width, decoded.height, decoded.bitDepth, decoded.windowMin, decoded.windowMax, maxSize);
